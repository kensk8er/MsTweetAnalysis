"""
Executable file to perform item-popularity-based collaborative filtering. (Baseline algorithm)

* Almost equivalent to contentBased.userPopularity.py model.
"""
import graphlab as gl
from sklearn.metrics import precision_score
from module.ML.CF import ItemPopularity
...
if __name__ == '__main__':
    ...
    (train_set, test_set) = ItemPopularity.train_test_split(pref_sf=pref_sf, test_ratio=0.1)

    logging.info("Deleting users who have applied less than %s jobs." % min_app_num)
    test_set, test_user_ids = ItemPopularity.remove_user_by_count(data_set=test_set, min_count=min_app_num)

    logging.info("Converting test set into array format...")
    test_set = ItemPopularity.sf2array(sf=test_set, shape=pref_data.shape, row_key='user_id', col_key='job_id')

    logging.info("Hide %s of ratings from test set..." % (1 - reveal_ratio))
    test_data, test_labels = ItemPopularity.test_test_split(test_set=test_set, reveal_ratio=reveal_ratio,
    ...
    test_data = ItemPopularity.array2sf(array=test_data, row_key='user_id', col_key='job_id')

    logging.info("Learning item-based collaborative filtering model...")
    item_pop_model = gl.recommender.create(train_set.append(test_data), 'user_id', 'job_id', method='popularity',
                                           verbose=False)

    logging.info("Computing scores for test data...")
    job_num = len(pref_sf['job_id'].unique())
    scores = item_pop_model.recommend(users=test_user_ids, k=job_num, exclude_known=False)
    scores = scores[scores['score'] > 0]

    logging.info("Converting scores into array format...")
    scores = ItemPopularity.sf2array(sf=scores, shape=pref_data.shape, row_key='user_id', col_key='job_id',
    ...
    predictions = ItemPopularity.predict(scores=scores, labels=test_labels, possible_labels=possible_labels)
