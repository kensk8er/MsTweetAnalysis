...
from module.ML.CF import ItemBased
...
if __name__ == '__main__':
    ...
    count = 0
    for i in range(10):
        (train_set, test_set) = ItemBased.train_test_split(pref_sf=pref_sf, test_ratio=0.1)

        logging.info("Deleting users who have applied less than %s jobs." % min_app_num)
        test_set, test_user_ids = ItemBased.remove_user_by_count(data_set=test_set, min_count=min_app_num)

        logging.info("Converting test set into array format...")
        test_set = ItemBased.sf2array(sf=test_set, shape=pref_data.shape, row_key='user_id', col_key='job_id')

        logging.info("Hide %s of ratings from test set..." % (1 - reveal_ratio))
        test_data, test_labels = ItemBased.test_test_split(test_set=test_set, reveal_ratio=reveal_ratio,
        ...
        test_data = ItemBased.array2sf(array=test_data, row_key='user_id', col_key='job_id')
        ...
        scores = ItemBased.sf2array(sf=scores, shape=pref_data.shape, row_key='user_id', col_key='job_id',
        ...
        predictions = ItemBased.predict(scores=scores, labels=test_labels, possible_labels=possible_labels)
