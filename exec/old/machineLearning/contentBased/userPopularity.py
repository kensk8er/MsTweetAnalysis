...
if __name__ == '__main__':
    ...
    try:
        valid_user_ids = unpickle('tmp/userPopularity_valid_user_ids.pkl')
        ...
        logging.info('The number of users: %s' % len(valid_user_ids))
        enpickle(valid_user_ids, 'tmp/userPopularity_valid_user_ids.pkl')
    ...
    valid_job_ids = Popularity.get_valid_job_ids(pref_data, min_app_num=min_job_app_num)
    ...
    logging.info('The number of users: %s' % len(valid_user_ids))
    logging.info('The number of jobs: %s' % len(valid_job_ids))
    logging.info('The number of applications: %s' % pref_data.sum())

    try:
        labels = unpickle('tmp/userPopularity_labels.pkl')
    except:
        logging.info('Getting labels...')
        labels = Popularity.get_labels(pref_data=pref_data)
        enpickle(labels, 'tmp/userPopularity_labels.pkl')
    ...
    count = 0
    for i in range(10):
        predictions = Popularity.predict(popularity=popularity, labels=test_labels,
                                         possible_labels=possible_labels, binarizer=mlb)
        # test_predictions = Popularity.predict_k(popularity=popularity, sample_num=len(test_labels), binarizer=mlb, k=100)
    ...
    enpickle(avg_test_precision, 'sandbox/precision.pkl')
